...
class DumbPoem extends MeteredPoem{
     ...
     */
    void Article(){
        int x = (int)(Math.random() * 4); // random # 0, 1 or 2
        switch(x){
            case 0:
                stack.push("a");
                break;
    ...
    void Adjective(){
        ...
        for( String w : Poet.wordMap.keySet()){
            if(Poet.wordMap.get(w).partOfSpeech.contains("A")){
                adjs.add(w);
            }
        }
    ...
    void Noun(){
        ...
        for( String w : Poet.wordMap.keySet()){
            if(Poet.wordMap.get(w).partOfSpeech.contains("N")){
                nouns.add(w);
            }
        }
    ...
    void Verb(String choice){
        ...
        for (String w : Poet.wordMap.keySet()){
            if (Poet.wordMap.get(w).partOfSpeech.contains("i")){
                intrans.add(w);
            }
            if (Poet.wordMap.get(w).partOfSpeech.contains("Vt")){
                trans.add(w);
            }
        }
