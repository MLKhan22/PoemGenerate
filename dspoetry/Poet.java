...
class Poet{
	...
	/*
	 * Minh: Testing method for Final Project
	 * 
	 */
	static String writeLineWithMeterAndAlliteration(String meter, String lastWord){
		// Add words whose prefixes match what we are looking for.
		String rv = "";		
		while(meter.length() > 0){
			String nextWord = findWordWithStressPrefixAndPhone(meter, lastWord);				
			meter = meter.substring(numSyllables(nextWord));
			rv = rv + " " + nextWord; // + concatenates strings
		}
		return rv + " " + lastWord;
	 ...
	 * 
	 */
	private static String findWordWithStressPrefixAndPhone(String meter, String lastWord) {
		String[] lastPhones = wordMap.get(lastWord).phonemes;
		int N = wordList.length;
		int i = (int)(N * Math.random());
		for(int j = 0; j < N; j++){
			int index = (i + j) % N;
			String[] phonemes = wordMap.get(wordList[index].word).phonemes;
			if(wordList[index].initialMeterMatch(meter) && phonemes[0].equals(lastPhones[0]) && !wordList[index].word.equals(lastWord))
    ...
    static String writeLineWithMeterAndRhymeswith(String meter, String target){
    	...
    	String lastWord = target.substring(target.lastIndexOf(" ")+1);
        String targetPhonemes = getTargetPhonemes(target);
        String rhymingWord = "";
        Word[] testList = wordList;
        shuffleArray(testList);
        for( Word w : testList ){
            if( w.endsWith(targetPhonemes) && endMeterMatch(meter, w.meter) && !w.word.equals(lastWord)){
                rhymingWord = w.word;
